...
-- ==================================================
-- 사용법:
-- 1. 인덱스 없는 상태에서 쿼리 복사해서 MySQL에 실행 (EXPLAIN 이미 포함)
...
EXPLAIN SELECT * FROM users WHERE public_id = (SELECT public_id FROM users LIMIT 1);

-- 1-2. email로 조회 (idx_email 인덱스 필요)
EXPLAIN SELECT * FROM users WHERE email = 'hong@example.com';

-- 1-3. 잔액 범위 조회
EXPLAIN SELECT * FROM users WHERE balance >= 100000 ORDER BY balance DESC;
...
EXPLAIN SELECT * FROM categories WHERE name = '전자제품';

-- 2-2. 카테고리별 상품 수 집계
EXPLAIN SELECT c.name, COUNT(p.id) as product_count
FROM categories c
...
EXPLAIN SELECT * FROM products WHERE name LIKE '%노트북%';

-- 3-3. 가격 범위 조회
EXPLAIN SELECT * FROM products WHERE price BETWEEN 10000 AND 100000;

-- 3-4. 재고가 있는 상품 조회
EXPLAIN SELECT * FROM products WHERE stock > 0 ORDER BY stock DESC;

-- 3-5. 카테고리별 평균 가격
EXPLAIN SELECT c.name, AVG(p.price) as avg_price, COUNT(p.id) as product_count
FROM categories c
...
EXPLAIN SELECT * FROM shipping_addresses WHERE user_id = 1;

-- 4-2. 기본 배송지 조회 (idx_user_default 복합 인덱스 필요)
EXPLAIN SELECT * FROM shipping_addresses WHERE user_id = 1 AND is_default = TRUE;

-- 4-3. 사용자별 배송지 개수
EXPLAIN SELECT user_id, COUNT(*) as address_count
...
EXPLAIN SELECT * FROM coupons WHERE coupon_type = 'RATE';
...
-- 5-4. 대기열 사용 쿠폰 조회
EXPLAIN SELECT * FROM coupons WHERE use_queue = TRUE;
...
EXPLAIN SELECT uc.*, c.name, c.discount_rate, c.discount_amount
FROM user_coupons uc
JOIN coupons c ON uc.coupon_id = c.id
WHERE uc.user_id = 1 AND uc.status = 'AVAILABLE';

-- 6-2. 쿠폰 ID로 발급 내역 조회 (idx_coupon_id 인덱스 필요)
EXPLAIN SELECT * FROM user_coupons WHERE coupon_id = 1;

-- 6-3. 만료 예정 쿠폰 조회 (idx_expires_at 인덱스 필요)
EXPLAIN SELECT uc.*, u.name, c.name as coupon_name
FROM user_coupons uc
...
-- 6-4. 사용자별 쿠폰 사용 통계
EXPLAIN SELECT user_id, status, COUNT(*) as count
FROM user_coupons
...
EXPLAIN SELECT * FROM coupon_queues
...
-- 7-4. 쿠폰별 대기 현황 통계
EXPLAIN SELECT coupon_id, status, COUNT(*) as count, MIN(queue_position) as min_pos, MAX(queue_position) as max_pos
FROM coupon_queues
...
EXPLAIN SELECT * FROM orders WHERE order_number = (SELECT order_number FROM orders LIMIT 1);
...
-- 8-5. 사용자별 주문 통계
EXPLAIN SELECT user_id, COUNT(*) as order_count, SUM(final_amount) as total_amount
FROM orders
GROUP BY user_id
ORDER BY total_amount DESC;

-- 8-6. 상태별 주문 통계
EXPLAIN SELECT status, COUNT(*) as count, SUM(final_amount) as total_amount
FROM orders
GROUP BY status;

-- 8-7. 일별 주문 통계
EXPLAIN SELECT DATE(created_at) as order_date, COUNT(*) as order_count, SUM(final_amount) as daily_revenue
FROM orders
...
EXPLAIN SELECT oi.*, p.name, p.price
FROM order_items oi
JOIN products p ON oi.product_id = p.id
WHERE oi.order_id = 1;

-- 9-2. 상품별 판매 내역 (idx_product_id 인덱스 필요)
EXPLAIN SELECT product_id, SUM(quantity) as total_sold, SUM(subtotal) as total_revenue
FROM order_items
GROUP BY product_id
...
-- 9-3. 베스트셀러 상품 조회
EXPLAIN SELECT p.name, p.category_id, SUM(oi.quantity) as total_sold
FROM order_items oi
JOIN products p ON oi.product_id = p.id
...
EXPLAIN SELECT * FROM payments WHERE payment_id = (SELECT payment_id FROM payments LIMIT 1);

-- 10-2. 주문별 결제 조회 (idx_order_id 인덱스 필요)
EXPLAIN SELECT * FROM payments WHERE order_id = 1;

-- 10-3. 결제 상태별 조회 (idx_status 인덱스 필요)
EXPLAIN SELECT * FROM payments WHERE status = 'COMPLETED';

-- 10-4. 데이터 전송 상태별 조회 (idx_data_transmission_status 인덱스 필요)
EXPLAIN SELECT * FROM payments WHERE data_transmission_status = 'PENDING';

-- 10-5. 결제 완료 건수 및 금액 집계
EXPLAIN SELECT status, COUNT(*) as count, SUM(paid_amount) as total_amount
FROM payments
GROUP BY status;

-- 10-6. 일별 결제 통계
EXPLAIN SELECT DATE(created_at) as payment_date,
...
EXPLAIN SELECT ci.*, p.name, p.price, (p.price * ci.quantity) as subtotal
FROM cart_items ci
JOIN products p ON ci.product_id = p.id
WHERE ci.user_id = 1;

-- 11-2. 상품별 장바구니 담김 수 (idx_product_id 인덱스 필요)
EXPLAIN SELECT product_id, COUNT(*) as cart_count, SUM(quantity) as total_quantity
FROM cart_items
GROUP BY product_id
ORDER BY cart_count DESC;

-- 11-3. 특정 상품을 담은 사용자 조회 (UNIQUE KEY uk_user_product 활용)
EXPLAIN SELECT ci.*, u.name, u.email
FROM cart_items ci
...
EXPLAIN SELECT * FROM refunds WHERE order_id = 2;

-- 12-2. 환불 상태별 조회 (idx_status 인덱스 필요)
EXPLAIN SELECT * FROM refunds WHERE status = 'COMPLETED';

-- 12-3. 환불 통계
EXPLAIN SELECT status, COUNT(*) as count, SUM(refund_amount) as total_refund
FROM refunds
GROUP BY status;

-- 12-4. 환불 사유별 통계
EXPLAIN SELECT reason, COUNT(*) as count, SUM(refund_amount) as total_amount
FROM refunds
GROUP BY reason
ORDER BY count DESC;
...
-- 13-4. 이벤트 타입별 통계
EXPLAIN SELECT event_type, status, COUNT(*) as count
FROM outbox_events
...
EXPLAIN SELECT * FROM popular_products
WHERE `rank` <= 10
ORDER BY `rank`;
...
-- 14-3. 카테고리별 인기 상품
EXPLAIN SELECT category_name, COUNT(*) as count, AVG(total_sales_quantity) as avg_sales
...
WHERE o.id IN (
    EXPLAIN SELECT DISTINCT order_id
    FROM order_items
    WHERE product_id = 1
);
